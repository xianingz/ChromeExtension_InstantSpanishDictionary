...
function ajaxGet(data, _b){
    return new Promise(function (resolve, reject) {
        chrome.runtime.sendMessage({action: 'ajaxGet', data: data}, function (response) {
            ...
            $obj.find("._3ne2Sqvz").remove();
			$obj.find("._6AhgG5IT").remove();
			$obj.find("._3QFIA64h").remove();
				
		    var ex='';
		    if(te1){
			    ex += '<span class="maintitle">';
			    ex += '<a  class="maintitle" href="https://www.spanishdict.com/translate/'+te1+'" target="_blank">'+te1+':</a>';
			    ex += '</span>';
			    ex += '<span class="maintitle">';
			    ex += te3;
			    ex += '</span>';
			    ex += '<div class="example d_balloon_content">';
			    ex += te4[0].outerHTML;
			    ex += '</div>';
			
		    }

		    $('#' + balloon + '_define', _b.balloon).html(ex + '<iframe style="width:1px;height:1px;display:none" src="' + ga + '&referer=' + window.location.href + '&app_name=Chrome EXT" frameborder="0" width="1" height="1" scrolling="no"></iframe>');
			
		    addToHistory(_b.word, window.location.href);
		    ...
		    $(response).remove();
		    $('#d_balloon .d_balloon_content *').css('font-size', _b.options.font_size + 'px');
...
(function (ns) {
	...
	_b.xhr = function (opt, resrc) {
		...
		return ajaxGet({
			...
			//url: $protocol + '://' + host + '/translate/' + encodeURIComponent(opt.selection),
			url: 'https://www.spanishdict.com/translate/' + encodeURIComponent(opt.selection),
			dataType: 'HTML',
			timeout: 35000,
			cache: true,
			headers: headers,
	...
	_b.render = function (resp, status, xhr) {
		...
		$('#' + balloon + '_define', this.balloon).html(ex + '<iframe style="width:1px;height:1px;display:none" src="' + ga + '?path=' + path + '&referer=' + window.location.href + '&app_name=Chrome EXT" frameborder="0" width="1" height="1" scrolling="no"></iframe>');
